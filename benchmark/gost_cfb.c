...
void CFB_blk_encrypt(WORD *preBlkCipher, WORD* currBlkPlain, WORD* CipherText) {
    gostcrypt_1RD(preBlkCipher, key, CipherText);
    xor_buf(currBlkPlain, CipherText, GOST_BLOCK_SIZE);
    mm_memcpy(preBlkCipher, CipherText, GOST_BLOCK_SIZE);
}
...
void my_main(){
    ...
    WORD preBlkCipher[GOST_BLOCK_SIZE] = {0x3abdf7e0, 0xe643a43d};;
