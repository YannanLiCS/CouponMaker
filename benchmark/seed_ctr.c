...
void CTR_Blk_encrypt(BYTE *CNT, BYTE *currBlkPlain, BYTE *CipherText){
	seed_encrypt(CNT, CipherText);
    xor_buf(currBlkPlain, CipherText, SEED_BLOCK_SIZE);
    increment_CNT(CNT, SEED_BLOCK_SIZE);
}
...
void my_main(){
    ...
    for (i = 0; i < 256/SEED_BLOCK_SIZE; i++) {
    	CTR_Blk_encrypt(CNT, currBlkPlain, Cipher);
    }
}
