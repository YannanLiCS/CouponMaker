...
typedef unsigned long u32;       // Should be 32-bit = 4 BYTEs, change to "long" for 16-bit machines
...
static const u32 keyschedule[32] = {
	0x56be4a0f, 0xe9f62877, 0x68bcb66c, 0x078911dd, 0x5b82740b, 0xfd24d09b,
	0x8d608015, 0xa120e0be, 0x810a75ae, 0x1bf223e5,	0xf9c0d2d0, 0x0f676c02, 
	0x8f9b5c84, 0x8a7c8ddd, 0xd4ab4896, 0x18e93447, 0xcf090f51, 0x5a4c8202,
	0x4ec3196f, 0x61b1a0dc, 0x244e07c1, 0xd0d10b12, 0x69917c6c, 0x7ff94fb3,
	0x9a7eb482, 0x723b5738, 0xb97522c5, 0x39cc6349, 0xffc2afd5, 0x1412e731,
	0xa9af7241, 0xa3e67359
};
...
#define OP(X1, X2, X3, X4, rbase)				\
    t0.w = X3 ^ keyschedule[rbase];			\
    t1.w = X4 ^ keyschedule[rbase+1];			\
    t1.w ^= t0.w;						\
...
static void
seed_encrypt(const BYTE *inbuf, BYTE *outbuf)
{
  u32 x1, x2, x3, x4;
  union wordbuf t0, t1;
...
static void
seed_decrypt(const BYTE *inbuf, BYTE *outbuf)
{
  u32 x1, x2, x3, x4;
  union wordbuf t0, t1;
...
void CFB_blk_encrypt(BYTE *preBlkCipher, BYTE* currBlkPlain, BYTE *Cipher) {
  	seed_encrypt(preBlkCipher, Cipher);
    xor_buf(currBlkPlain, Cipher, SEED_BLOCK_SIZE);
    mm_memcpy(preBlkCipher, Cipher, SEED_BLOCK_SIZE);
}
...
void my_main(){
    ...
    BYTE preBlkCipher[SEED_BLOCK_SIZE] = {
        0xf6, 0x43, 0x25, 0x1c, 0xd2, 0x43, 0x4, 0x4,
        0xdc, 0xa2, 0xda, 0xb1, 0xca, 0xa6, 0x77, 0x60
    };
    BYTE currBlkPlain[SEED_BLOCK_SIZE] = {
        0xe2, 0x70, 0x8c, 0x91, 0x35, 0xff, 0xdf, 0xa5,
        0x6b, 0x76, 0xdd, 0x55, 0xef, 0xf4, 0x6c, 0x28
    };
    BYTE Cipher[SEED_BLOCK_SIZE];
    int i;
    for (i = 0; i < 256/SEED_BLOCK_SIZE; i++) {
    	CFB_blk_encrypt(preBlkCipher, currBlkPlain, Cipher);
    }
}
