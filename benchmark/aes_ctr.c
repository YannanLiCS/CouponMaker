...
void CTR_Blk_encrypt(BYTE *CNT, BYTE *currBlkPlain, BYTE *Cipher){
    aes_encrypt(CNT, key_schedule, 256, Cipher);
    xor_buf(currBlkPlain, Cipher, AES_BLOCK_SIZE);
    increment_CNT(CNT, AES_BLOCK_SIZE);
}
...
void my_main(){
    ...
    for (i = 0; i < 256 / AES_BLOCK_SIZE; i++) {
    	CTR_Blk_encrypt(CNT, msg, Cipher);
    }
}
