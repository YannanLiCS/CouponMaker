...
void ccm_prepare_first_ctr_blk(BYTE counter[], const BYTE nonce[], int nonce_len, int payload_len_store_size)
{
    mm_memset(counter, 0, AES_BLOCK_SIZE);
    counter[0] = (payload_len_store_size - 1) & 0x07;
    mm_memcpy(&counter[1], nonce, nonce_len);
}
...
void ccm_prepare_first_format_blk(BYTE buf[], int assoc_len, int payload_len, int payload_len_store_size, int mac_len, const BYTE nonce[], int nonce_len)
{
    ...
    mm_memcpy(&buf[1], nonce, nonce_len);
    mm_memset(&buf[1 + nonce_len], 0, AES_BLOCK_SIZE - 1 - nonce_len);
...
void ccm_format_assoc_data(BYTE buf[], int *end_of_buf, const BYTE assoc[], int assoc_len)
{
    ...
    *end_of_buf += 2;
    mm_memcpy(&buf[*end_of_buf], assoc, assoc_len);
    *end_of_buf += assoc_len;
    pad = AES_BLOCK_SIZE - (*end_of_buf % AES_BLOCK_SIZE); /*BUG?*/
    mm_memset(&buf[*end_of_buf], 0, pad);
    *end_of_buf += pad;
}
...
void ccm_format_payload_data(BYTE buf[], int *end_of_buf, const BYTE payload[], int payload_len)
{
    ...
    mm_memcpy(&buf[*end_of_buf], payload, payload_len);
...
void aes_encrypt(const BYTE in[], const WORD key[], int keysize, BYTE *out)
{
    BYTE state[4][4];
...
WORD key_schedule[60] = {
    ...
    0x2d9810a3, 0x914dff4, 0x9ba35411, 0x8e6925af, 0xa51a8b5f, 0x2067fcde,
    ...
    0x268c3ba7, 0x9e04214, 0x68007bac, 0xb2df3316, 0x96e939e4, 0x6c518d80,
...
void CFB_blk_encrypt(BYTE *preBlkCipher, BYTE* currBlkPlain, BYTE *Cipher) {
    aes_encrypt(preBlkCipher, key_schedule, 256, Cipher);
    xor_buf(currBlkPlain, Cipher, AES_BLOCK_SIZE);
    mm_memcpy(preBlkCipher, Cipher, AES_BLOCK_SIZE);
}
