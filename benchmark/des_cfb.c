...
void InvIP(WORD state[], BYTE out[])
{
    out[0] = BITNUMINTR(state[1],7,7) | BITNUMINTR(state[0],7,6) | BITNUMINTR(state[1],15,5) |
              BITNUMINTR(state[0],15,4) | BITNUMINTR(state[1],23,3) | BITNUMINTR(state[0],23,2) |
              BITNUMINTR(state[1],31,1) | BITNUMINTR(state[0],31,0);

    out[1] = BITNUMINTR(state[1],6,7) | BITNUMINTR(state[0],6,6) | BITNUMINTR(state[1],14,5) |
              BITNUMINTR(state[0],14,4) | BITNUMINTR(state[1],22,3) | BITNUMINTR(state[0],22,2) |
              BITNUMINTR(state[1],30,1) | BITNUMINTR(state[0],30,0);

    out[2] = BITNUMINTR(state[1],5,7) | BITNUMINTR(state[0],5,6) | BITNUMINTR(state[1],13,5) |
              BITNUMINTR(state[0],13,4) | BITNUMINTR(state[1],21,3) | BITNUMINTR(state[0],21,2) |
              BITNUMINTR(state[1],29,1) | BITNUMINTR(state[0],29,0);

    out[3] = BITNUMINTR(state[1],4,7) | BITNUMINTR(state[0],4,6) | BITNUMINTR(state[1],12,5) |
              BITNUMINTR(state[0],12,4) | BITNUMINTR(state[1],20,3) | BITNUMINTR(state[0],20,2) |
              BITNUMINTR(state[1],28,1) | BITNUMINTR(state[0],28,0);

    out[4] = BITNUMINTR(state[1],3,7) | BITNUMINTR(state[0],3,6) | BITNUMINTR(state[1],11,5) |
              BITNUMINTR(state[0],11,4) | BITNUMINTR(state[1],19,3) | BITNUMINTR(state[0],19,2) |
              BITNUMINTR(state[1],27,1) | BITNUMINTR(state[0],27,0);

    out[5] = BITNUMINTR(state[1],2,7) | BITNUMINTR(state[0],2,6) | BITNUMINTR(state[1],10,5) |
              BITNUMINTR(state[0],10,4) | BITNUMINTR(state[1],18,3) | BITNUMINTR(state[0],18,2) |
              BITNUMINTR(state[1],26,1) | BITNUMINTR(state[0],26,0);

    out[6] = BITNUMINTR(state[1],1,7) | BITNUMINTR(state[0],1,6) | BITNUMINTR(state[1],9,5) |
              BITNUMINTR(state[0],9,4) | BITNUMINTR(state[1],17,3) | BITNUMINTR(state[0],17,2) |
              BITNUMINTR(state[1],25,1) | BITNUMINTR(state[0],25,0);

    out[7] = BITNUMINTR(state[1],0,7) | BITNUMINTR(state[0],0,6) | BITNUMINTR(state[1],8,5) |
...
void des_crypt(const BYTE in[], const BYTE key[][6], BYTE out[])
{
    WORD state[2],idx,t;
    IP(state,in);
...
void three_des_crypt(const BYTE in[], const BYTE key[][16][6], BYTE out[])
{
    des_crypt(in, key[0], out);
    des_crypt(out, key[1], out);
    des_crypt(out, key[2], out);
}
...
BYTE key_schedule[16][6] = {
    0xb, 0x2, 0x67, 0x9b, 0x49, 0xa5,
    0x69, 0xa6, 0x59, 0x25, 0x6a, 0x26,
    0x45, 0xd4, 0x8a, 0xb4, 0x28, 0xd2,
    0x72, 0x89, 0xd2, 0xa5, 0x82, 0x57,
    0x3c, 0xe8, 0x3, 0x17, 0xa6, 0xc2,
    0x23, 0x25, 0x1e, 0x3c, 0x85, 0x45,
    0x6c, 0x4, 0x95, 0xa, 0xe4, 0xc6,
    0x57, 0x88, 0x38, 0x6c, 0xe5, 0x81,
    0xc0, 0xc9, 0xe9, 0x26, 0xb8, 0x39,
    0x91, 0xe3, 0x7, 0x63, 0x1d, 0x72,
    0x21, 0x1f, 0x83, 0xd, 0x89, 0x3a,
    0x71, 0x30, 0xe5, 0x45, 0x5c, 0x54,
    0x91, 0xc4, 0xd0, 0x49, 0x80, 0xfc,
    0x54, 0x43, 0xb6, 0x81, 0xdc, 0x8d,
    0xb6, 0x91, 0x5, 0xa, 0x16, 0xb5,
    0xca, 0x3d, 0x3, 0xb8, 0x70, 0x32
};
...
void CFB_Blk_encrypt(BYTE *preBlkCipher, BYTE *currBlkPlain, BYTE *CipherText){
    three_des_crypt(preBlkCipher, &key_schedule, CipherText);
    xor_buf(currBlkPlain, CipherText, DES_BLOCK_SIZE);
    mm_memcpy(preBlkCipher, CipherText, DES_BLOCK_SIZE);
}
...
void my_main(){
    ...
    BYTE preBlkCipher[DES_BLOCK_SIZE] = {0xf6, 0x43, 0x25, 0x1c, 0xd2, 0x43, 0x4, 0x4};
    BYTE currBlkPlain[DES_BLOCK_SIZE] = {0xe2, 0x70, 0x8c, 0x91, 0x35, 0xff, 0xdf, 0xa5};
    BYTE CipherText[DES_BLOCK_SIZE];
    int i;
    for (i = 0; i < 256/DES_BLOCK_SIZE; i++) {
    	CFB_Blk_encrypt(preBlkCipher, currBlkPlain, CipherText);
    }
}
