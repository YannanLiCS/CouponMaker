...
typedef unsigned long u32;             // 32-bit u32, change to "long" for 16-bit machines
...
static const BLOWFISH_context context = {
	{
...
static void encrypt_block (BLOWFISH_context *context, BYTE *outbuf, const BYTE *inbuf)
{
...
void CFB_Blk_encrypt(BYTE *preBlkCipher, BYTE *currBlkPlain, BYTE *CipherText){
    do_decrypt_block(&context, CipherText, preBlkCipher);
    xor_buf(currBlkPlain, CipherText, BLOWFISH_BLOCK_SIZE);
    mm_memcpy(preBlkCipher, CipherText, BLOWFISH_BLOCK_SIZE);
}
...
void my_main(){
    ...
    BYTE preBlkCipher[BLOWFISH_BLOCK_SIZE] = {
        0xf6, 0x43, 0x25, 0x1c, 0xd2, 0x43, 0x4, 0x4
    };
    BYTE currBlkPlain[BLOWFISH_BLOCK_SIZE] = {
        0xe2, 0x70, 0x8c, 0x91, 0x35, 0xff, 0xdf, 0xa5
    };
    BYTE CipherText[BLOWFISH_BLOCK_SIZE];
    int i;
    for (i = 0; i < 100; i++) {
